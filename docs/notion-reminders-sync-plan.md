...
- Production Notion-to-Apple creates are explicit via `icloudbridge reminders notion-production-create-apple`; it only runs for allowlisted production mappings and requires exact `--expected-creates`, a `--max-creates` cap, `--apply`, and `--confirm-production`.
